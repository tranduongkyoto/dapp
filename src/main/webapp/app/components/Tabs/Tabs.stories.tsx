    ...
    <TabList tabStyle="bar" isVertical={false} defaultActiveKey={1}>
      <Tab
        tabName={
          ...
          >
            <Icon fill="black" size={22} svg={iconTypes.messageCircle} /> <span style={{ paddingLeft: '4px' }}>Freinds </span>
          </div>
        }
        tabKey={1}
      >
      ...
      <Tab
        ...
        tabKey={2}
      >
        <Button icon={iconTypes.plus} text="Add New Alert" theme="primary" type="button" onClick={() => {}} />
      </Tab>
    </TabList>
  ),
};
...
BulbWithIcon.args = {
  ...
  children: (
    <TabList tabStyle="bulbUnion" defaultActiveKey={1}>
      <TabList.Tab tabName={<Icon fill="black" size={16} svg={iconTypes.grid} />} tabKey={1} lineHeight={0}>
        <div>This is Card 1</div>
      </TabList.Tab>
      <TabList.Tab tabName={<Icon fill="black" size={16} svg={iconTypes.list} />} lineHeight={0} tabKey={2}>
        <div>This is Card 2</div>
      </TabList.Tab>
    </TabList>
  ),
