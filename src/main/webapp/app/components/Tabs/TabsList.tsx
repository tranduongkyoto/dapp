...
import { BulbTab, StyledTab, StyledTabBar, StyledTabContent, StyleTabBarParent } from './Tabs.styles';
...
const getTabs = (children: JSX.Element) => {
  ...
  React.Children.forEach(children, (element: IElement) => {
    if (element.type.displayName === 'Tab') {
...
function TabList({
  ...
  const [tabChildren, setTabChildren] = useState<any>(getTabs(children && (children as any)));
  ...
  return (
    ...
    >
      <StyleTabBarParent isVertical={isVertical} data-testid="tabs_list_parent" {...props}>
        <StyledTabBar haveBackground={tabStyle == 'bulbUnion'} isVertical={isVertical}>
          {children}
        </StyledTabBar>
        <StyledTabContent isVertical={isVertical}>{tabChildren[selectedKey]}</StyledTabContent>
      </StyleTabBarParent>
    </TabContext.Provider>
  );
}

export function Tab({ activeState, isDisabled = false, lineHeight = 24, tabKey, tabName }: ITab) {
  ...
  const RenderBulbTab = () => {
    return (
      <BulbTab
        role="tab-Item"
        data-testid={`tab_item_${tabKey}_${activeState ? activeState : tabKey == selectedKey}`}
        key={tabKey}
  ...
  const RenderBarTab = () => {
    return (
      <StyledTab
        ...
        isDisabled={isDisabled}
      >
        <span data-testid={`disabled_${isDisabled}_${tabKey}`}>{tabName}</span>
      </StyledTab>
    );
  };
